...
export default async (req, res) => {
    ...
    try {
        const { challengeID, eventName, date, time, locale } = await req.body
        ...
        if (req.headers.cookie) {
            ...
            if (rtsEventCookie) {
                const url = `${serverRuntimeConfig.API_BASE_URL}${serverRuntimeConfig.API_STAGE}/events/${eventName}/${userID}/challenges/${challengeID}/start?lang=${locale}${finalDate ? `&ts=${finalDate}` : ''}`
                ...
                if (response.status === 200) {
                    ...
                    throw new Error('There is a problem with the start quiz fetch')
                }
            } else {
