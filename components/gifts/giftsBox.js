...
function GiftsBox (props) {
    ...
    function giftIcon (i) {
        return (
            <SvgIcon key={i} viewBox="0 0 34 34" className={classes.gift}>
    ...
    function getGifts (gifts) {
        ...
        for (let i = 0; i < gifts.length; i++) {
            if (gifts[i].type === 'lottery') {
                gifts[i].locked ? re.push(lockedGiftIcon(i)) : re.push(giftIcon(i))
            } else if (gifts[i].type === 'medal') {
                gifts[i].locked ? re.push(disabledMedalIcon(i)) : re.push(medalIcon(i))
            }
        }
        return re
    }
    ...
    return (
        <Box className={classes.container}>
            <Typography className={classes.textRegularCenter}>
                Vos recompenses{ /* `${gameStats.totalChallengesCount} ${translation.dashBoardChallengesOfTheDay}` */ }
            </Typography>
