    ...
    function getGifts (gifts, props) {
        ...
        for (let i = 0; i < gifts.length; i++) {
            if (gifts[i].type === 'lottery') {
                gifts[i].locked ? re.push(lockedGiftIcon({ key: i, onClick: props.onClick, id: gifts[i].giftID, setGift: props.setGift })) : re.push(giftIcon({ key: i, onClick: props.onClick, id: gifts[i].giftID, setGift: props.setGift }))
            } else if (gifts[i].type === 'medal') {
                gifts[i].locked ? re.push(disabledMedalIcon({ key: i, onClick: props.onClick, id: gifts[i].giftID, setGift: props.setGift })) : re.push(medalIcon({ key: i, onClick: props.onClick, id: gifts[i].giftID, setGift: props.setGift }))
            }
        }
        return re
    }
